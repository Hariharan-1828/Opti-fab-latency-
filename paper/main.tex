...
We deploy a MobileNetV2 backbone~\cite{sandler2018mobilenetv2} pretrained on ImageNet~\cite{deng2009imagenet} as our feature extractor. Because MobileNetV2 requires 3-channel RGB inputs, we prepended a grayscale-to-RGB adapter consisting of a $1 \times 1$ Convolution layer with 3 filters:
\begin{equation}
...
During training, we freeze the first 100 layers of the MobileNetV2 base and fine-tune the remaining layers using the Adam optimizer~\cite{kingma2014adam} ($lr = 1\times 10^{-4}$) under a balanced class weight loss function to handle minor dataset imbalances.
...
We define the normalized Shannon entropy $H$~\cite{shannon1948mathematical} of the prediction to measure uncertainty:
\begin{equation}
...
For borderline cases where the fast path output triggers a deferral or requires formal safety validation, the system switches to the MC Dropout path~\cite{gal2016dropout}. The inference graph is reconstructed by forcing the Keras dropout layer active (\texttt{training=True}). 
