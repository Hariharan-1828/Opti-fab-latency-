"""
OPTI-FAB — TensorRT Benchmark
Benchmarks inference latency across three execution providers:
  1. CPU (baseline)
  2. CUDA (onnxruntime-gpu)
  3. TensorRT (onnxruntime + TRT execution provider)

Produces a clean results table saved to benchmarks/trt_results.txt

Usage:
    python benchmarks/benchmark_trt.py
"""
...
WARMUP_RUNS  = 50     # Discard first N runs (GPU warm-up, JIT compilation)
BENCH_RUNS   = 500    # Number of timed runs per provider
...
def benchmark_provider(provider: str, session_options=None) -> dict:
    """
    Runs inference BENCH_RUNS times on a given execution provider.
...
def get_trt_provider_options():
    """TensorRT execution provider options for onnxruntime."""
...
def run_benchmark():
    if not MODEL_ONNX.exists():
        ...
        log.error("Run export_onnx.py first to generate the model.")
        return
    ...
    # 3. TensorRT (first run builds and caches the engine — takes ~60s)
    log.info("TensorRT: first run will compile engine (~60s). Subsequent runs use cache.")
    ...
    header = (
        f"\n{'='*70}\n"
        f"  OPTI-FAB — Inference Benchmark Results\n"
